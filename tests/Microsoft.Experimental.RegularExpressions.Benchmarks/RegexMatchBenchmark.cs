...
namespace Microsoft.Experimental.RegularExpressions.Benchmarks
{
    ...
    //[MemoryDiagnoser] - no memory is alloc'd (for now)
    public class RegexMatchBenchmark
    {
        // From the C# spec, these are the allowed characters
        // Letter character - Lu, Ll, Lt, Lm, Lo, or Nl 
        // Mn or Mc 
        // Nd 
        // Pc
        // Cf
        // From .Net,
        // \w ==> [\p{Ll}\p{Lu}\p{Lt}\p{Lo}\p{Nd}\p{Pc}\p{Lm}
        // \d ==> \p{Nd}
        ...
        private const string MetadataKeyRegexForSept09 = @"^(" + StartCharacterRegex + ")(" + IdentifierCharacterRegexForSept09 + ")*$";

        private static readonly STRE.Regex MetadataKeyRegexForSept09_STRE = new STRE.Regex(
            MetadataKeyRegexForSept09);

        private static readonly PCRE.Regex MetadataKeyRegexForSept09_PCRE = new PCRE.Regex(
            MetadataKeyRegexForSept09);

        private static readonly STRE.Regex MetadataKeyRegexForSept09_STRE_Compiled = new STRE.Regex(
            MetadataKeyRegexForSept09,
            STRE.RegexOptions.Compiled);

        private static readonly PCRE.Regex MetadataKeyRegexForSept09_PCRE_Compiled = new PCRE.Regex(
            MetadataKeyRegexForSept09,
            PCRE.RegexOptions.Compiled);

        public IEnumerable<object> MatchIdentifierArguments()
        {
            yield return "a";
            yield return "aaaaaaaaaaa";
        ...
        //    MetadataKeyRegexForSept09_STRE.IsMatch(value);

        //[Benchmark]
        //[ArgumentsSource(nameof(MatchIdentifierArguments))]
        //public bool MatchIdentifier_PCRE(string value) =>
        //    MetadataKeyRegexForSept09_PCRE.IsMatch(value);

        //[Benchmark]
        //[ArgumentsSource(nameof(MatchIdentifierArguments))]
        //public bool MatchIdentifier_STRE_Compiled(string value) =>
        //    MetadataKeyRegexForSept09_STRE_Compiled.IsMatch(value);

        //[Benchmark]
        //[ArgumentsSource(nameof(MatchIdentifierArguments))]
        //public bool MatchIdentifier_PCRE_Compiled(string value) =>
        //    MetadataKeyRegexForSept09_PCRE_Compiled.IsMatch(value);

        //[Benchmark]
        //[ArgumentsSource(nameof(MatchIdentifierArguments))]
        //public bool MatchIdentifier_Roslyn(string value) =>
        //    UnicodeCharacterUtilities.IsValidIdentifier(value);

        // The following works around https://github.com/dotnet/BenchmarkDotNet/issues/1198
        private static readonly string testInput = new string('a', 128);

        [Benchmark]
        public bool MatchIdentifier_PCRE() =>
           MetadataKeyRegexForSept09_PCRE_Compiled.IsMatch(testInput);
    }
