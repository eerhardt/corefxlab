...
{
    // Comparison table of .NET vs PCRE
    ...
    public class RegexMatchTests // : RemoteExecutorTestBase
    {
        public static IEnumerable<object[]> Match_Basic_TestData()
        {
            ...
            // Actual - "(?<char>\\w)\\<char>"
            // disabled -- PCRE requires \k<char>
            //yield return new object[] { @"(?<char>\w)\<char>", "aa", RegexOptions.None, 0, 2, true, "aa" };
            yield return new object[] { @"(?<char>\w)\k<char>", "aa", RegexOptions.None, 0, 2, true, "aa" };

            // Actual - "(?<43>\\w)\\43"
            // disabled -- "group name must start with a non-digit"
            ...
            // Bug? Should work, as we pass PCRE2_UTF
            ...
            // PCRE does not support ECMAScript
            ...
            // disabled: PCRE does not support implicit lookahead conditional
            // "reference to nonexistent subpattern"
            ...
            // PCRE does not support invariant culture
        ...
        {
            Assert.False(Regex.IsMatch("a", @"a{65535,}"));
            Assert.Throws<ArgumentException>("pattern", () => Regex.IsMatch("a", @"a{65536,}"));
        }
        ...
        {
            Assert.Throws<ArgumentException>("pattern", () => new Regex("unmatched((paren"));
        }
    }
}
